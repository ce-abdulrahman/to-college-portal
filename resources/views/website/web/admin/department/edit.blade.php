            ...
            <div class="page-title-box d-flex align-items-center justify-content-between">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">نوێ کردنی بەش</li>
                    </ol>
                </div>
                <h4 class="page-title">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <select id="system_id" name="system_id" class="form-select" required>
                                    @foreach ($systems as $system)
                                        <option value="{{ $system->id }}" {{ old('system_id', $department->system_id) == $system->id ? 'selected' : '' }}>
                                            {{ $system->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select id="province_id" name="province_id" class="form-select" required>
                                    @foreach ($provinces as $province)
                                        <option value="{{ $province->id }}" {{ old('province_id', $department->province_id) == $province->id ? 'selected' : '' }}>
                                            {{ $province->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select id="university_id" name="university_id" class="form-select" required>
                                    @foreach ($universities as $university)
                                        <option value="{{ $university->id }}" {{ old('university_id', $department->university_id) == $university->id ? 'selected' : '' }}>
                                            {{ $university->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3">
                                ...
                                <select id="type" name="type" class="form-select">
                                    <option value="زانستی" {{ old('type', $department->type) === 'زانستی' ? 'selected' : '' }}>زانستی</option>
                                    <option value="وێژەیی" {{ old('type', $department->type) === 'وێژەیی' ? 'selected' : '' }}>وێژەیی</option>
                                    <option value="زانستی و وێژەیی" {{ old('type', $department->type) === 'زانستی و وێژەیی' ? 'selected' : '' }}>هەردوو</option>
                                </select>
                            </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <select id="sex" name="sex" class="form-select">
                                    <option value="نێر" {{ old('sex', $department->sex) === 'نێر' ? 'selected' : '' }}>نێر</option>
                                    <option value="مێ" {{ old('sex', $department->sex) === 'مێ' ? 'selected' : '' }}>مێ</option>
                                </select>
                            </div>
                                        ...
                                        <div class="row g-3 mt-3">
                                            <div class="col-md-6">
                                                ...
                                                <input id="lat" name="lat" type="number" step="0.000001"
                                                       value="{{ old('lat', $department->lat) }}" class="form-control" readonly>
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label">Longitude</label>
                                                <input id="lng" name="lng" type="number" step="0.000001"
                                                       value="{{ old('lng', $department->lng) }}" class="form-control" readonly>
                                            </div>
                                        </div>
                            ...
                            <div class="col-12">
                                ...
                                <textarea id="description" name="description" class="form-control" rows="4">{{ old('description', $department->description) }}</textarea>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select id="status" name="status" class="form-select" required>
                                    <option value="1" {{ old('status', $department->status) == 1 ? 'selected' : '' }}>چاڵاک</option>
                                    <option value="0" {{ old('status', $department->status) == 0 ? 'selected' : '' }}>ناچاڵاک</option>
                                </select>
                            </div>
