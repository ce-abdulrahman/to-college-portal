          ...
          <table id="datatable" class="table align-middle nowrap" style="width:100%">
            ...
            <tbody>
              @foreach ($colleges as $index => $college)
                <tr
                  data-province-id="{{ $college->university->province_id }}"
                ...
                >
                  ...
                  <td class="actions">
                    <a href="{{ route('admin.colleges.show', $college->id) }}" class="btn btn-sm btn-outline-info" data-bs-toggle="tooltip" data-bs-title="پیشاندان">
                      <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.colleges.edit', $college->id) }}" class="btn btn-sm btn-outline-primary" data-bs-toggle="tooltip" data-bs-title="دەستکاری">
                      <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                    <form action="{{ route('admin.colleges.destroy', $college->id) }}" method="POST" class="d-inline" onsubmit="return confirm('دڵنیایت؟');">
                      @csrf @method('DELETE')
                      <button type="submit" class="btn btn-sm btn-outline-danger" data-bs-toggle="tooltip" data-bs-title="سڕینەوە">
...
@push('scripts')
  {{-- API route (ئارامی‌تر لە hardcode) --}}
  <script>window.UNI_API = "{{ route('admin.api.universities') }}";</script>
