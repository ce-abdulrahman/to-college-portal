                    ...
                    <div class="col-md-3">
                        ...
                        <select id="filter-university" class="form-select">
                            ...
                            @foreach ($universities as $uni)
                                <option value="{{ $uni->id }}" data-province="{{ $uni->province_id }}">
                                    {{ $uni->name }}</option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <tbody>
                            @foreach ($colleges as $college)
                                <tr data-province="{{ $college->university->province_id ?? '' }}"
                                    data-university="{{ $college->university_id }}" data-status="{{ $college->status }}">
                                    <td>{{ $loop->iteration }}</td>
                                    <td>
                                        <img src="{{ $college->image }}" alt="{{ $college->name }}" class="rounded"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function filterTable() {
                ...
                table.rows().every(function() {
                    ...
                    if (searchVal && !row.text().toLowerCase().includes(searchVal.toLowerCase())) show =
                        false;
            ...
            provinceFilter.on('change', function() {
                ...
                if (provinceId) {
                    universityFilter.find('option:not([data-province="' + provinceId + '"]):not(:first)')
                        .hide();
                }
