...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="rounded-2xl border bg-white p-4">
            <h2 class="text-lg font-semibold">زانکۆ: {{ $university->name }}</h2>
            ...
            <form class="mt-4 space-y-4" method="POST" action="{{ route('admin.geo.university.update-geo', $university) }}"
                enctype="multipart/form-data">
                @csrf @method('PUT')

                <label class="block text-sm font-medium">GeoJSON (اختیاری)</label>
                <textarea name="geojson_text" rows="8" class="w-full rounded-xl border p-2"
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input id="lat" name="lat" value="{{ old('lat', $university->lat) }}"
                            class="w-full rounded-xl border p-2" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium">Longitude</label>
                        <input id="lng" name="lng" value="{{ old('lng', $university->lng) }}"
                            class="w-full rounded-xl border p-2" />
                    </div>
                </div>
    ...
    <script>
        ...
        @if ($university->lat && $university->lng)
            marker = L.marker([{{ $university->lat }}, {{ $university->lng }}]).addTo(markerLayer);
