                        ...
                        <div class="card-body">
                            ...
                            @if ($existingRequest && $existingRequest->isPending())
                                ...
                                <div class="text-center mt-4">
                                    <form method="POST" action="{{ route('center.features.cancel-request', $existingRequest->id) }}"
                                        class="d-inline">
                                        @csrf
                                        @method('DELETE')
                            ...
                            @else
                                @php
                                    ...
                                    $limitTeacherUnitPrice = (int) ($limitPrices['teacher'] ?? $defaultLimitPrices['teacher']);
                                ...
                                <div class="alert alert-warning">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            ...
                                            <p class="mb-0">دەتوانیت داوای تایبەتمەندی یان زیادکردنی سنوور بنێریت.</p>
                                            <p class="mb-0 mt-2">داواکاریەکەت دەنێردرێت بۆ بەڕێوەبەری سیستم.</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="card border-light mb-4">
                                    <div class="card-body">
                                        <h6 class="mb-3"><i class="fas fa-gauge me-2"></i>دۆخی سنوورەکان</h6>
                                        <div class="row g-3">
                                            <div class="col-md-6">
                                                <div class="small text-muted">سنووری مامۆستا</div>
                                                <div class="fw-semibold">
                                                    {{ is_null($center->limit_teacher) ? 'بێ سنوور' : $center->limit_teacher }}
                                                    @if (!is_null($center->limit_teacher))
                                                        <span class="text-muted"> (بەکارهاتوو: {{ $currentTeachersCount ?? 0 }})</span>
                                                    @endif
                                                </div>
                                            </div>
                                            <div class="col-md-6">
                                                ...
                                                <div class="fw-semibold">
                                                    ...
                                                    @if (!is_null($center->limit_student))
                                                        <span class="text-muted"> (بەکارهاتوو: {{ $currentStudentsCount ?? 0 }})</span>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <div class="card bg-soft-info border-info-soft border-dashed mb-4 shadow-none">
                                            <div class="card-body p-3">
                                                <div class="ms-1">
                                                    ...
                                                    <p class="mb-2 small text-muted lh-lg">
                                                        نرخی زیادکردنی سنوور: هەر 1 مامۆستا =>
                                                        {{ number_format($limitTeacherUnitPrice) }} دینار، هەر 1 قوتابی =>
                                        ...
                                        <div class="row">
                                            <div class="col-md-3 mb-3">
                                                <div class="card h-100 border-{{ $center->all_departments == 1 ? 'success' : 'warning' }}">
                                                    <div class="card-body">
                                            ...
                                            <div class="col-md-3 mb-3">
                                                <div class="card h-100 border-{{ $center->ai_rank == 1 ? 'success' : 'warning' }}">
                                                    <div class="card-body">
                                                        <div class="form-check">
                                                            <input class="form-check-input" type="checkbox"
                                            ...
                                            <div class="col-md-3 mb-3">
                                                <div
                                                    class="card h-100 border-{{ $center->queue_hand_department == 1 ? 'success' : 'warning' }}">
                                                    <div class="card-body">
                                                        <div class="form-check">
                                                            <input class="form-check-input" type="checkbox"
                                                                name="request_types[]" value="queue_hand_department"
                                                                id="queueHandDepartmentCheck"
                                                                {{ $center->queue_hand_department == 1 ? 'disabled checked' : '' }}>
                                        ...
                                        <div class="row g-3 mt-1">
                                            <div class="col-md-6">
                                                <label for="request_limit_teacher" class="form-label fw-bold">
                                                    <i class="fas fa-user-tie me-1"></i>زیادکردنی سنووری مامۆستا
                                                </label>
                                                <input type="number" min="0" class="form-control"
                                                    id="request_limit_teacher" name="request_limit_teacher"
                                                    value="{{ old('request_limit_teacher', 0) }}">
                                            </div>
                                            <div class="col-md-6">
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const limitTeacherUnitPrice = @json($limitTeacherUnitPrice ?? 5000);
            const limitStudentUnitPrice = @json($limitStudentUnitPrice ?? 1000);
            const requestLimitTeacherInput = document.getElementById('request_limit_teacher');
            ...
            function getLimitTeacherQty() {
                return Math.max(0, parseInt(requestLimitTeacherInput ? requestLimitTeacherInput.value : '0', 10) || 0);
            }
            ...
            function updatePrice() {
                ...
                const limitsPrice = (getLimitTeacherQty() * limitTeacherUnitPrice) + (getLimitStudentQty() * limitStudentUnitPrice);
            ...
            function hasAnyRequest() {
                ...
                return getCheckedTierFeatureCount() > 0 || hasQueueRequest || getLimitTeacherQty() > 0 || getLimitStudentQty() > 0;
            }

            checkboxes.forEach(cb => cb.addEventListener('change', updatePrice));
            if (requestLimitTeacherInput) requestLimitTeacherInput.addEventListener('input', updatePrice);
            ...
            if (requestLimitTeacherInput) {
                requestLimitTeacherInput.addEventListener('input', () => {
