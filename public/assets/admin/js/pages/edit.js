...
function initPageSpecificFeatures(page) {
    const features = {
        'departments': initDepartmentsEdit,
        'students': initStudentsEdit,
        'teachers': initTeachersEdit,
        'universities': initUniversitiesEdit,
        'colleges': initCollegesEdit,
        'provinces': initProvincesEdit
    };
...
function initDepartmentsEdit() {
    initDependentSelects();
    initLeafletMap();
    initSummernote();
}

function initStudentsEdit() {
    initReferralCodeSelect2();
}

function initTeachersEdit() {
    initTeacherValidation();
}

function initUniversitiesEdit() {
    initLeafletMap();
    initGeoJSONMap();
    initSummernote();
}

function initCollegesEdit() {
    initDependentSelects();
    initLeafletMap();
    initGeoJSONMap();
    initSummernote();
...
function initDependentSelects() {
    ...
    if (provinceSelect && universitySelect) {
        ...
        if (currentProvinceId) {
            loadUniversities(currentProvinceId, universitySelect);
        }
        ...
        provinceSelect.addEventListener('change', function() {
            ...
            if (provinceId) {
                loadUniversities(provinceId, universitySelect);
            }
        });
    }

    if (universitySelect && collegeSelect) {
        const currentUniversityId = universitySelect.value;
        if (currentUniversityId) {
            loadColleges(currentUniversityId, collegeSelect);
        }
        ...
        universitySelect.addEventListener('change', function() {
            ...
            if (universityId) {
                loadColleges(universityId, collegeSelect);
            }
        });
    }
}

function loadUniversities(provinceId, universitySelect) {
    fetch(`${window.API_UNI}?province_id=${provinceId}`)
        .then(response => response.json())
        .then(universities => {
            const currentValue = universitySelect.dataset.current;
            universities.forEach(uni => {
                const option = document.createElement('option');
                option.value = uni.id;
                option.textContent = uni.name;
                if (uni.id == currentValue) option.selected = true;
...
function loadColleges(universityId, collegeSelect) {
    fetch(`${window.API_COLLS}?university_id=${universityId}`)
        .then(response => response.json())
        .then(colleges => {
            const currentValue = collegeSelect.dataset.current;
            colleges.forEach(college => {
                const option = document.createElement('option');
                option.value = college.id;
                option.textContent = college.name;
                if (college.id == currentValue) option.selected = true;
...
function initGeoJSONMap() {
    ...
    if (geojsonTextarea.value) {
        try {
            const geojson = JSON.parse(geojsonTextarea.value);
