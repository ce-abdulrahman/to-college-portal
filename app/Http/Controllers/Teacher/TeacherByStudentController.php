...
class TeacherByStudentController extends Controller
{
    ...
    private function resolveFeatureFlags(array $data): array
    {
        ...
        foreach ($features as $feature) {
            $teacherHasFeature = (int) ($teacher?->{$feature} ?? 0) === 1;
            $requestedValue = (int) ($data[$feature] ?? 0) === 1 ? 1 : 0;

            $resolved[$feature] = $teacherHasFeature ? $requestedValue : 0;
        }
    ...
    public function index()
    {
        ...
        return view('website.web.teacher.student.index', compact('students'));
    }
    ...
    public function create()
    {
        $user = auth()->user();
        $teacher = $user?->teacher;
        $studentLimit = $teacher?->limit_student;
        ...
        return view('website.web.teacher.student.create', compact(
            'provinces',
            'studentLimit',
    ...
    public function show(Student $student)
    {
        ...
        return view('website.web.teacher.student.show', compact('user', 'student', 'result_deps', 'ai_rankings'));
    }
    ...
    public function edit(Student $student)
    {
        ...
        $user = auth()->user();
        $studentLimit = $user?->teacher?->limit_student;
        ...
        return view('website.web.teacher.student.edit', compact(
            'student',
            'provinces',
            'studentLimit',
            'activeStudentsCount',
    ...
    public function update(Request $request, Student $student)
    {
        ...
        $isCurrentStudentActive = (int) $student->status === 1 && (int) data_get($student, 'user.status', 0) === 1;
        $studentLimit = auth()->user()?->teacher?->limit_student;
